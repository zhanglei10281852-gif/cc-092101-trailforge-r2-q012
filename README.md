...
初始化命令可重复执行；已应用的迁移不会重复写入。SQLite 连接会启用外键约束、WAL、busy timeout 和写入重试。所有业务时间在数据库中保存为 UTC ISO-8601 文本（统一以 `Z` 结尾），API 接受任意带固定偏移量的时间并转换为等价的 UTC 时刻（可逆转换，保留绝对瞬间），拒绝不带时区的时间而不是猜测时区；旧库中的非 UTC 偏移文本由 `0002` 迁移安全归一化，遇到无法解析的行会中止迁移并报告表、列、行 id 与原始值，需人工修复后重跑。

迟到与超时分钟统一按秒级差值**向下取整**并在零处截断：不足 60 秒记 0 分钟，提前打卡永远不会得到负数分钟。超时风险阈值在边界分钟处**就高包含**：`>=30` 分钟 moderate、`>=120` high、`>=360` critical，其余 low。提交签到时不允许未来时间，打卡时刻必须落在活动窗口（集合/开始的较早者至结束）之内；重复打卡保持冲突、相同幂等键重复提交返回同一结果。
