...
全系统只有一套时间约定，由 `trailforge/timekeeping.py` 实现，请求校验、数据库编码、仓储筛选和服务计算共用：

- **存储**：所有业务时间在 SQLite 中保存为可逆的 UTC 文本 `YYYY-MM-DDTHH:MM:SS.ffffffZ`。同一物理瞬间无论用 `+08:00`、`-05:00` 还是 `Z` 提交，都归一化为同一文本，保存再读取后瞬间不变。
- **读取**：只接受带显式偏移的 ISO-8601 文本（`Z` 或 `±HH:MM`）。无时区文本不会被猜测为服务器本地时间（这正是夏令时切换周出现一小时偏差的根源），而是抛出携带表/列/行信息的 `UnparseableTimestampError`。
- **请求**：请求体和查询参数中的时间都必须带时区；缺少时区统一返回 422，带偏移的时间归一化为 UTC 后再比较，所有查询入口（超时列表、安全汇总、各项统计、审计筛选）行为一致。
- **秒换算分钟**：迟到/超时分钟数只计已完成的整分钟（非负秒数向下取整），提前或准点一律为 0，绝不出现负值。59 秒 → 0，60 秒 → 1。
- **风险阈值**：超时风险 `[0,30)` 低、`[30,120)` 中、`[120,360)` 高、`[360,∞)` 严重（边界值进入更高一档）；风险评估分值 `1–4` 低、`5–9` 中、`10–16` 高、`17–25` 严重。定义见 `trailforge/domain/risk.py`。

### 存量数据迁移

迁移 `0002` 把库中所有时间列规范化为上述 UTC 文本：已是规范格式的行保持不变，带显式偏移的旧文本（如 `2026-03-29T10:30:00+08:00` 或空格分隔形式）换算为同一瞬间的 UTC 文本。迁移在单个事务中执行，可安全重跑。无法解释的值（无时区或非法文本）不会被静默猜测：`init-db` 以非零退出码报告每一处的表、列、rowid、原值和原因，修正数据后重新执行即可：

```bash
python -m trailforge.cli init-db   # 失败时打印 JSON 形式的 failures 列表
```
